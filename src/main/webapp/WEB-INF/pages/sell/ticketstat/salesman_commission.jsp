...
<SCRIPT type="text/javascript">
    ...
    var formSetting = {
    		...
    		type:"query",
    		hiddens:["id"],
    	fields:[[{title:"所属部门",type:"pop",name:"deptId",fn:'selectOrg',map:orgMap},{title:"售票日期",type:"datescale",name:"createTime"},
    	...
    	queryRow:{align:'center',buttons:[
		                 			   {
		                 			    	 ...
		                 			    	 handlers:{
		                 			    		click: function(){
		    										...
		    										$("<form target='ef' action='${pageContext.request.contextPath}/public/stat/excelSalesmanCommision.htmls'></form>").appendTo($('body')).submit();
		                 			    		 }
		                 			    	 }
		                 			     }
		                 			 ]}	
    ...
    function queryData(e,queryForm){
    	...
    	var deptId = queryForm.find(":input[name=deptId]").val();
    	if(deptId!=''){
    		param.deptId = deptId;
    	} 
    	if(playDate!=''){
    	...
    	$.ajax({
    		url:"<%=path%>/public/stat/querySalesmanCommission.htmls",
    		type:"POST",
    		dataType:"json",
    		async:false,
    		data:param,
    		success:function(ret){
    			if(ret==''){
    				table.append($("<tr><td colspan='13'>没有数据！</td></tr>"));
    				return;
    			}
    			var tr = null;
    			...
    			for(var i=0;i<ret.length;i++){
    				...
    				amount += parseInt(fields[8]);
    				commision += parseInt(fields[10]);
    				totalNum += parseInt(fields[11]);
    ...
    function CreateOneFormPage(){
    	...
    	LODOP.ADD_PRINT_HTM(40,"11mm", div.width(), 39,
    			"<div style='width:"+div.width()+";text-align:center;font-size:13pt'>营销员提成明细表</div>");
    	//LODOP.ADD_PRINT_HTM(40,"11mm", div.width(), 39,
    	//		"<div style='font-weight:bold;font-size:16pt;width:"+div.width()+";text-align:center;'>售票按票类统计表</div>");
...
<th width="100">营销号</th>
<th width="100">营销员</th><th width="120">服务部门</th>
<th width="140">工作单位</th>
<th width="140">预订场所</th>
<th width="100">操作员</th>
<th width="100">消费金额</th><th width="100">提成比例</th>
