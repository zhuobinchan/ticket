...
@Entity
@Table(name = "bi_member_card_consumer")
@org.hibernate.annotations.Table(appliesTo = "bi_member_card_consumer", comment = "会员卡、消费卡、充值卡消费明细表")
public class MemberCardConsumer {
	@Id
	...
	@Column(nullable = false, columnDefinition = "DECIMAL(13,2) DEFAULT 0 COMMENT '消费金额'")
	@FieldDesc(comment = "消费金额")
	private Float money;

	@Column(nullable = false, columnDefinition = "DECIMAL(13,2) DEFAULT 0 COMMENT '本次消费后余额'")
	@FieldDesc(comment = "本次消费后余额")
	private Float balance;

	@Column(name = "branch", length = 8, columnDefinition = "VARCHAR(8) COMMENT '消费场所'")
